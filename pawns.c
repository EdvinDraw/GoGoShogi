...
void pawnupdate() BANKED{
	for(uint8_t x=1; x<6; x++){
		for(uint8_t y=1; y<6; y++){
			
			if (board[x + y * BOARD_W] == (PAWN | IS_WHITE)){
				whitethreat[(x) + (y+1) * BOARD_W] = 1;
			}
			else if (board[x + y * BOARD_W] == (PAWN)){
				blackthreat[(x) + (y-1) * BOARD_W] = 1;
			}
			else if (board[x + y * BOARD_W] == (PAWNPLUS | IS_WHITE)){
				whitethreat[(x) + (y-1) * BOARD_W] = 1;
			...
			else if (board[x + y * BOARD_W] == (PAWNPLUS)){
