...
void silverupdate() BANKED{
	for(uint8_t x=1; x<6; x++){
		for(uint8_t y=1; y<6; y++){
			
			if (board[x + y * BOARD_W] == (SILVER | IS_WHITE)){
			...
			else if (board[x + y * BOARD_W] == (SILVER)){
			...
			else if (board[x + y * BOARD_W] == (SILVERPLUS | IS_WHITE)){
				whitethreat[(x) + (y-1) * BOARD_W] = 1;
			...
			else if (board[x + y * BOARD_W] == (SILVERPLUS)){
				blackthreat[(x-1) + (y-1) * BOARD_W] = 1;
				blackthreat[(x) + (y-1) * BOARD_W] = 1;
				blackthreat[(x+1) + (y-1) * BOARD_W] = 1;
